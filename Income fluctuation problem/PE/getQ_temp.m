...
function [Q,pi] = getQ_temp(PS,PJ,p,x0,xGrid,gstjn,Gstj,zeta,h)
%% some error checking
...
if Nextra > 1 % nothing to do if Nextra = 1
    ...
    %}
    cCDF = (1 + temp/2 + ([0:Nextra-1]-1/2)*temp).^(-zeta); % complementary CDF
    % "-1/2" is important for adjusting for quadrature!
    r(1:end-1) = -diff(cCDF);
    r(end) = cCDF(end);
end
